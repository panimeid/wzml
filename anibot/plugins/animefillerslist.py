...
async def fillers_cmd(client: anibot, message: Message, mdata: dict):
    ...
    if len(qry)==1:
        return await message.reply_text("Berikan beberapa nama anime untuk mencari pengisi\ncontoh: /fillers Detective Conan")
    k = search_filler(qry[1])
    if k == {}:
        await message.reply_text("Tidak ada pengisi yang ditemukan untuk anime yang diberikan...")
        return
    button = []
    list_ = list(k.keys())
    if len(list_)==1:
