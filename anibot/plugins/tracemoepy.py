...
async def trace_bek(client: anibot, message: Message, mdata: dict):
    ...
    if dls_loc:
        async with session:
            ...
            caption_ = (
                f"**Judul**: `{result['anilist']['title']['romaji']}` - (**{result['anilist']['title']['native']}**)\n"
...
async def tracemoe_btn(client: anibot, cq: CallbackQuery, cdata: dict):
    ...
    if await check_if_adult(int(result['anilist']['id']))=="True" and await (SFW_GRPS.find_one({"id": cq.message.chat.id})):
        ...
        button.append([InlineKeyboardButton("⎇ Lihat Anime", callback_data=f"anime more {result['anilist']['id']} {user}"), InlineKeyboardButton(text="⌧ Hapus", callback_data=f'neko_delete, {user}')])
    if int(page)==0:
