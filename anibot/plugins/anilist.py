...
async def top_tags_cmd(client: Client, message: Message, mdata: dict):
    ...
    msg, buttons = result
    await message.reply_text(msg[0], disable_web_page_preview=True, reply_markup=buttons if buttons!='' else None)
...
async def favourites_cmd(client: Client, message: Message, mdata: dict):
    ...
    await message.reply_photo(result[0], caption="Pilih salah satu opsi di bawah ini", reply_markup=btn)
