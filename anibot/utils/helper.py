...
def get_btns(media, user: int, result: list, lsqry: str = None, lspage: int = None, auth: bool = False, sfw: str = "False"):
    ...
    if media == "ANIME" and sfw == "False":
        buttons.append([
            ...
            InlineKeyboardButton(text="Series", callback_data=f"ls_{result[2][0]}_ANI{qry}{pg}_{str(auth)}_{user}"),
        ])
    ...
    if auth is True and media!="SCHEDULED" and sfw == "False":
        ...
        if media=="ANIME" and sfw == "False":
            buttons.append([InlineKeyboardButton("Media", callback_data=f"media {result[2][0]}")])
...
def get_auth_btns(media, user, data, name, lsqry: str = None, lspage: int = None):
    ...
    if media=="CHARACTER":
        ...
        if media=="ANIME":
            btn.append(InlineKeyboardButton("Media", callback_data=f"media_login {result[2][0]}"))
